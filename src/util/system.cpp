...
const char * const BITCOIN_CONF_FILENAME = "ryanbucks.conf";
...
#else
    const char* pszModule = "ryanbucks";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Ryanbucks";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Ryanbucks";
#else
    // Unix
    return pathRet / ".ryanbucks";
#endif
#endif
}
