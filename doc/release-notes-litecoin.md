Ryanbucks Core version 0.18.1 is now available from:

  <https://download.ryanbucks.org/ryanbucks-0.18.1/>
...
To receive security and update notifications, please subscribe to:
  ...
  <https://groups.google.com/forum/#!forum/ryanbucks-dev>
...
`/Applications/Ryanbucks-Qt` (on Mac) or `ryanbucksd`/`ryanbucks-qt` (on
Linux).
...
Ryanbucks Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use Ryanbucks Core on unsupported systems.

Ryanbucks Core should also work on most other Unix-like systems but is not
as frequently tested on them.

From 0.17.0 onwards, macOS <10.10 is no longer supported. 0.17.0 is
built using Qt 5.9.x, which doesn't support versions of macOS older than
10.10. Additionally, Ryanbucks Core does not yet change appearance when
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/ryanbucks-project/ryanbucks/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](mailto:contact@ryanbucks.org) the Ryanbucks Core
  project to let us know about their use-case, as this feature could be
  deprecated in the future).

Wallet changes
--------------
When creating a transaction with a fee above `-maxtxfee` (default 0.1 RXB),
the RPC commands `walletcreatefundedpsbt` and  `fundrawtransaction` will now fail
instead of rounding down the fee. Beware that the `feeRate` argument is specified
in RXB per kilobyte, not litoshi per byte.

Documentation
-------------

- A new short [document](https://github.com/ryanbucks-project/ryanbucks/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/ryanbucks-project/ryanbucks/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/ryanbucks-project/ryanbucks/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/ryanbucks-project/ryanbucks/blob/master/doc/ryanbucks-conf.md)
  about the `ryanbucks.conf` file describes how to use it to configure
  Ryanbucks Core.

- A new document introduces Ryanbucks Core's BIP174 [Partially-Signed
  Ryanbucks Transactions
  (PSBT)](https://github.com/ryanbucks-project/ryanbucks/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/ryanbucks-project/ryanbucks/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Ryanbucks-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Ryanbucks Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/ryanbucks-project/ryanbucks/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/ryanbucks-project/ryanbucks/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Ryanbucks Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Ryanbucks Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/ryanbucks-project/ryanbucks/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/ryanbucks-project/ryanbucks/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart ryanbucksd with the `-deprecatedrpc=generate`
...
- A new `ryanbucks-wallet` tool is now distributed alongside Ryanbucks
...
This section describes planned changes to Ryanbucks Core that may affect
other Ryanbucks software and services.

- Since version 0.16.0, Ryanbucks Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Ryanbucks Core 0.20 (expected about
  a year after 0.18), Ryanbucks Core will default to native segwit
  ...
  to bech32 addresses, and if the Ryanbucks Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Ryanbucks Core 0.20.
  ...
  set the `addresstype=bech32` configuration option in any Ryanbucks Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Ryanbucks Core
...
- A sub-project of Ryanbucks Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Ryanbucks Core.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Ryanbucks Core's own implementation, although entropy
  gathered by Ryanbucks Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Ryanbucks Core a
...
- On macOS, Ryanbucks Core now opts out of application CPU throttling
